...
/* END */
		...
		Position
		FindLin(ElementType Key, HashTable H)
		{
			Position CurrentPos;
			int CollisionNum;
...
/* 3*/      while (H->TheCells[CurrentPos].Info != Empty &&
				...
				/* Probably need strcmp!! */
			{
/* 4*/          CurrentPos = Hash(Key, H->TableSize)+ ++CollisionNum;
/* 5*/          if (CurrentPos >= H->TableSize)
/* 6*/              CurrentPos -= H->TableSize;
			}
/* 7*/      return CurrentPos;
...
/* START: fig5_17.txt */
		...
		void
		InsertLin(ElementType Key, HashTable H)
		{
			Position Pos;
...
/* END */
		...
		int Collisions(HashTable H) {
			int Coll=0;
            int i;
			for ( i = 0; i < H->TableSize; i++) {
				if (i != Hash(H->TheCells[i].Element, H->TableSize) ) {
					if(H->TheCells[i].Info!=Empty)
 					Coll++;
